...
class BinaryClassification:
    ...
    def plot_confusion_matrix(
        self, threshold=None, normalize=False, title='Confusion matrix', cmap=plt.cm.Greys,
    ...
    #                    loc_legend='lower right'):
    def plot_roc_curve(
        self, threshold=None, plot_threshold=True, linewidth=2, y_text_margin=0.05, x_text_margin=0.2,
        ...
        if plot_threshold:
            ...
            if idx_thresh > 0.5 and idy_thresh > 0.5:
                plt.text(
                    x=idx_thresh - x_text_margin, y=idy_thresh - y_text_margin,
    ...
    def plot_precision_recall_curve(
        self, threshold=None, plot_threshold=True, beta=1, linewidth=2,
        fscore_iso=[0.2, 0.4, 0.6, 0.8], iso_alpha=0.7, y_text_margin=0.03,
        ...
        # Plot mean precision
        l, = plt.plot(
            [0, 1], [mean(prec), mean(prec)], color=c_mean_prec,
        ...
        if len(fscore_iso) > 0:  # Check to plot or not the fscore-iso
            for f_score in fscore_iso:
                ...
                l, = plt.plot(
                    x[y >= 0], y[y >= 0], color=c_f1_iso, linestyle=ls_fscore_iso,
                    alpha=iso_alpha,
                )
    ...
    def plot_class_distribution(
        self, threshold=None, display_prediction=True, alpha=.5, jitter=.3, pal_colors=None,
        display_violin=True, c_violin='white', strip_marker_size=4, strip_lw_edge=None,
        ...
        if display_prediction:
            sns.stripplot(
                x='class', y='pred', hue='type', data=pred_df,
    ...
    def plot_score_distribution(
        self, threshold=None, plot_hist_TN=True, kde_ksw_TN={'shade': True},
        ...
        ax = sns.distplot(
            TN_pred, hist=plot_hist_TN, kde_kws=kde_ksw_TN, label=label_TN,
            color=c_TN_curve,
        )
        # Plot False Positive predictions using hatch
        kde_x, kde_y = ax.lines[0].get_data()
        ax.fill_between(
            kde_x, kde_y, where=(kde_x >= t),
    ...
    #                    ls_bestthresh_line=':'):
    def plot_threshold(
        self, threshold=None, beta=1, title=None,
        annotation=True, bbox_dict=None, bbox=True, arrow_dict=None, arrow=True,
        plot_fscore=True, plot_recall=True, plot_prec=True, plot_fscore_max=True,
        ...
        if plot_recall:
            plt.plot(
                thresh, recall, label=label_recall,
        ...
        if plot_prec:
            plt.plot(
                thresh, precision, label=label_prec,
        ...
        if plot_fscore:
            ...
            plt.plot(
                thresh, fscore, label=label_fscr,
        ...
        if plot_fscore_max:
            plt.plot(
                x_max_fscore, y_max_fscore, marker=marker_fscore_max,
...
class MultiClassClassification:
    ...
    def plot_roc(self, threshold=None, linewidth=2, show_threshold=False):
        ...
        for i, color in zip(range(self.n_classes), cycle(colors)):
            plt.plot(
                fpr[i], tpr[i], color=color, lw=linewidth, alpha=.5,
